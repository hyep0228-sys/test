...
맨 위의 **「오답률 높은 순」** 표는 많이 틀린 문항부터 세운다(정답률 오름차순과 같은 순서다).
**중간·기말 출제 때 참고하려는 것**이다(교수자 용도). 처음에는 「문항 난이도 순」에 정답률만 적었는데,
교수자가 "오답률 높은 순서도 보여달라"고 해 **오답률을 앞에 세웠다** — 순서는 그대로고 표기만 바꿨다(2026-09-19).
